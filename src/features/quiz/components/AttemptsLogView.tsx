    ...
    >
      ...
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pt-8 pb-4">
        ...
        <div className="relative flex flex-col items-center justify-center text-center mt-2 mb-8">
          {/* Decorative Stars */}
          <div className="absolute -top-4 right-1/4 text-yellow-400 text-xl animate-pulse">
            ⭐
          </div>
          <div className="absolute top-2 left-1/4 text-yellow-400 text-lg animate-pulse">
            ⭐
          </div>
          <div className="absolute top-12 -left-2 text-yellow-400 text-2xl hidden md:block">
            ⭐
          </div>
      ...
      <main className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pb-12 flex-1 flex flex-col items-center">
          ...
          <div className="flex flex-col gap-3 mt-2">
            <AnimatePresence mode="popLayout">
              {paginatedAttempts.map((row, idx) => {
                ...
                // Expressive emoji for highest score
                const highestEmoji = getScoreEmoji(highestScore);

                // Alternating styling as shown in screenshot:
                // idx % 2 === 0 -> Solid Purple Row
                // idx % 2 === 1 -> White Row with purple text & subtle styling
      ...
      <div
        className="w-full h-16 bg-no-repeat bg-cover bg-bottom opacity-90 pointer-events-none mt-4"
        style={{
          backgroundImage:
            "radial-gradient(ellipse 100% 100% at 50% 100%, #6D28D9 0%, transparent 70%)",
        }}
      />
    </div>
  );
