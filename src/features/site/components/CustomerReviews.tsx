...
const renderReviewCard = (review: ReviewItem) => (
  ...
  >
    <div className="w-full flex justify-between items-center mb-3">
      ...
      {/* Star Rating based on review score */}
      <div className="flex items-center justify-start gap-1">
        {Array.from({ length: 5 }).map((_, index) => (
          <svg
            key={index}
            className={`w-4 h-4 md:w-5 md:h-5 ${
              index < review.rating ? "text-mad-third fill-mad-third" : "text-gray-300"
            }`}
            viewBox="0 0 20 20"
            fill="currentColor"
          >
    ...
    {review.title && (
      <h3 className="mad-label-1 font-bold text-mad-text-primary mb-2 text-start">
        {review.title}
      </h3>
    )}
    ...
    <div className="mt-auto flex flex-col items-start text-start">
      <h4 className="mad-label-1 text-mad-main font-bold mb-0.5">
        {review.author}
      </h4>
      {(review.subtitle || review.role) && (
        <span className="mad-body-3 text-xs text-mad-text-secondary font-medium">
          {review.subtitle || review.role}
        </span>
      )}
    </div>
...
const CustomerReviews: React.FC<CustomerReviewsProps> = ({
  ...
  const description = propDescription ?? testimonialsData?.description ?? "";
  ...
  const reviews: ReviewItem[] =
    ...
    (testimonialsData?.items?.map((item) => ({
      ...
      comment: item.quote,
      rating: typeof item.review === "number" ? item.review : 5,
    })) ?? []);
