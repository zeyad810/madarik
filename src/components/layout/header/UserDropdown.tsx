...
export const UserDropdown: React.FC<UserDropdownProps> = ({ onOpenSearch }) => {
  ...
  const isStudentUser =
    isStudent || userRole === "student" || activeAccount?.type === "student";
        ...
        {isOpen && (
          ...
          >
            {isStudentUser ? (
              <>
                {/* Student Info Row */}
                <div className="w-full flex items-center justify-between p-3 rounded-2xl bg-[#F7F5FF] border border-[#A855F7]/40 shadow-xs text-right">
                  <div className="flex items-center gap-3">
                    <div className="size-12 rounded-full ring-2 ring-purple-600 p-0.5 overflow-hidden shrink-0 bg-purple-50">
                      <Image
                        src={currentAvatarSrc}
                        alt={activeAccount?.name || "الطالب"}
                        width={48}
                        height={48}
                        className="size-full object-cover rounded-full"
                      />
                    ...
                    <span className="font-bold text-gray-900 text-base truncate">
                      {activeAccount?.name || "حساب الطالب"}
                    </span>
                  </div>
                  <span className="text-xs font-semibold px-3 py-1 rounded-full bg-[#DCFCE7] text-[#16A34A]">
                    نشط
                  </span>
                </div>
